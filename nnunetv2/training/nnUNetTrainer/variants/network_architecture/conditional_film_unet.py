...
class FiLMStage(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cond_vec: torch.Tensor) -> torch.Tensor:
        gamma_beta = self.affine(cond_vec)
...
class ConditionalFiLMUNet(nn.Module):
    """
    Wrapper over nnUNet backbone (encoder+decoder) with FiLM conditioning.

    - condition provided: returns binary logits (2 channels), with deep supervision behavior
      identical to the wrapped decoder.
    - condition omitted: runs condition sweep and returns multiclass logits where channel 0
      is background and channels in `condition_label_values` receive per-condition fg logits.
    """
    ...
    def _multiclass_forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        for cond_idx in range(self.num_conditions):
            ...
            out_bin = self._binary_forward(x, cond)
            per_condition_outputs.append(out_bin)
        ...
        if isinstance(per_condition_outputs[0], list):
            ...
            for s in range(n_scales):
                fg_list = []
                for out_bin in per_condition_outputs:
                    logits_bin = out_bin[s]
                    fg_list.append(logits_bin[:, 1] - logits_bin[:, 0])
        ...
        fg_list = []
        for out_bin in per_condition_outputs:
            fg_list.append(out_bin[:, 1] - out_bin[:, 0])
        fg_logits = torch.stack(fg_list, dim=1)
    ...
    def forward(
        ...
        return self._binary_forward(x, condition)
