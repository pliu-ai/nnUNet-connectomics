...
def compute_group_confusion_from_logits(
    ...
    active_eval_channels = [ECS_CHANNEL, PM_CHANNEL, CYTO_CHANNEL, ER_MEM_CHANNEL, ER_LUM_CHANNEL, NUCPL_CHANNEL]
    ...
    cond_tp = np.zeros((3,), dtype=np.float64)
    cond_fp = np.zeros((3,), dtype=np.float64)
    cond_fn = np.zeros((3,), dtype=np.float64)
    for slot_idx, channel_idx in enumerate((COND_SLOT_1_CHANNEL, COND_SLOT_2_CHANNEL, COND_SLOT_3_CHANNEL)):
...
def empty_validation_accumulators() -> Dict[str, np.ndarray]:
    ...
    return {
        ...
        "class_fp": np.zeros((NUM_OUTPUT_CHANNELS,), dtype=np.float64),
        "class_fn": np.zeros((NUM_OUTPUT_CHANNELS,), dtype=np.float64),
        "cond_tp": np.zeros((NUM_DYNAMIC_GROUPS, 3), dtype=np.float64),
        "cond_fp": np.zeros((NUM_DYNAMIC_GROUPS, 3), dtype=np.float64),
        "cond_fn": np.zeros((NUM_DYNAMIC_GROUPS, 3), dtype=np.float64),
