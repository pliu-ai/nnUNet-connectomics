...
class StructuredConditionalLoss(nn.Module):
    ...
    - optional conditional-group focus loss
    ...
    def _conditional_group_loss(
        ...
        """
        Binary Dice over the union of active conditional slots:
        p_group = p_slot1 + p_slot2, gt_group = gt_slot1 OR gt_slot2.
        """
        if not torch.any(active_conditional_slots):
            return self._safe_zero(zero_ref)

        slot_probs = torch.stack(
            (
        ...
        group_probs = (slot_probs * active).sum(dim=1, keepdim=True)

        target_group = ((target == COND_SLOT_1_CHANNEL) | (target == COND_SLOT_2_CHANNEL)).float()
        valid = valid_mask.float()

        axes = tuple(range(2, probs.ndim))
        intersect = (group_probs * target_group * valid).sum(dim=axes)
        pred_sum = (group_probs * valid).sum(dim=axes)
        gt_sum = (target_group * valid).sum(dim=axes)

        active_entries = active_conditional_slots.any(dim=1)
        ...
        if self.config.batch_dice:
            ...
            valid_group = active_entries.to(torch.int64).max()
            if self.config.ddp and dist.is_available() and dist.is_initialized():
                dist.all_reduce(valid_group, op=dist.ReduceOp.MAX)
            if not bool(valid_group.item()):
                return self._safe_zero(zero_ref)
        ...
        dice = (2.0 * intersect + self.config.smooth) / (pred_sum + gt_sum + self.config.smooth).clamp_min(1e-8)
        if not torch.any(active_entries):
            return self._safe_zero(zero_ref)
        return 1.0 - dice[active_entries].mean()
    ...
    def forward(
        ...
        group_loss = self._safe_zero(logits)
        if self.config.enable_group_focus and self.config.lambda_group > 0:
            group_loss = self._conditional_group_loss(probs, target, valid_mask, active_conditional_slots, logits)
        ...
        total = (
            ...
            + self.config.lambda_group * group_loss
