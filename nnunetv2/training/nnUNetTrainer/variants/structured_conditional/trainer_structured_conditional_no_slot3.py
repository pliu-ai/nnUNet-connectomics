...
from .dataloader_structured_conditional_no_slot3 import StructuredConditionalDataLoader3D
from .inference_structured_conditional_no_slot3 import (
...
from .label_mapping_no_slot3 import (
    ...
    original_label_name_lookup,
    remap_original_to_structured,
    sample_group_id_for_case,
)
from .metrics_structured_conditional_no_slot3 import (
...
from .structured_loss_no_slot3 import StructuredConditionalLoss, StructuredLossConfig


class nnUNetTrainerStructuredConditionalNoSlot3(nnUNetTrainer):
    """
    ...
    def __init__(
        ...
        # Optional global batch size override (before dataloader creation).
        # This overrides plans batch_size for this trainer only.
        self.global_batch_size_override = int(os.environ.get("NNUNET_STRUCTCOND_BATCH_SIZE", "0"))
        if self.global_batch_size_override < 0:
            raise ValueError("NNUNET_STRUCTCOND_BATCH_SIZE must be >= 0.")
        if self.global_batch_size_override > 0:
            self.configuration_manager.configuration["batch_size"] = int(self.global_batch_size_override)
        ...
        # Optional training length override for this trainer only.
        ...
        self.val_present_only = str(os.environ.get("NNUNET_STRUCTCOND_VAL_PRESENT_ONLY", "0")).lower() in {"1", "true", "yes", "y"}
    ...
    def _setup_wandb(self) -> None:
        ...
        run = wandb.init(
            ...
            config={
                ...
                "batch_size_global_override": self.global_batch_size_override,
                "batch_size_per_gpu_effective": self.batch_size,
                ...
                "val_present_only": self.val_present_only,
            },
        )
    ...
    def _log_wandb_epoch(self) -> None:
        ...
        report = self._latest_structured_val_report if isinstance(self._latest_structured_val_report, dict) else {}
        summary = report.get("summary", {}) if isinstance(report.get("summary", {}), dict) else {}
        mean_dice_key = "mean_original31_present_dice" if self.val_present_only else "mean_original31_dice"
        payload = {
            "epoch": epoch_idx,
            "train/loss": float(logs["train_losses"][-1]),
            "val/loss": float(logs["val_losses"][-1]),
            "val/mean_original31_dice": float(summary.get(mean_dice_key, logs["mean_fg_dice"][-1])),
            "lr": float(logs["lrs"][-1]),
        }

        original31_dice = report.get("original31_dice", [])
        if isinstance(original31_dice, (list, tuple)) and len(original31_dice) > 0:
            label_names = original_label_name_lookup()
            for idx, dice_value in enumerate(original31_dice, start=1):
                class_name = str(label_names.get(idx, f"label_{idx}"))
                payload[f"val/dice_original31/{idx:02d}_{class_name}"] = float(dice_value)
    ...
    def _filter_val_groups_to_present(self, group_ids: List[int], target) -> List[int]:
        if not self.val_present_only:
            return group_ids

        target_main = target[0] if isinstance(target, list) else target
        present_groups = set()
        for b in range(int(target_main.shape[0])):
        ...
        filtered = [int(group_id) for group_id in group_ids if int(group_id) in present_groups]
        # Fixed classes are condition-independent; evaluate one fallback group for fixed-only batches.
    ...
    def on_train_start(self):
        ...
        self.print_to_log_file(
            ...
            f"val_present_only={self.val_present_only}"
        )
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        group_ids_list = self._filter_val_groups_to_present(self._get_val_group_ids(), target)
        ...
        for group_id in group_ids_list:
            ...
            (
                ...
                valid_main,
                group_id=group_id,
                present_only=self.val_present_only,
            )
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        mean_active_dice_key = "mean_original31_present_dice" if self.val_present_only else "mean_original31_dice"
        mean_active_dice = float(report["summary"][mean_active_dice_key])
