...
DYNAMIC_GROUP_SPECS: Tuple[DynamicGroupSpec, ...] = (
    DynamicGroupSpec(0, "G1", "Mito", (4, 5), ("mito_mem", "mito_lum")),
...
def build_active_conditional_slot_mask(group_ids: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        Tensor with shape [B, 2], where True means the conditional slot is active
    ...
    active = torch.zeros((b, 2), dtype=torch.bool, device=group_ids.device)
    for i in range(b):
...
def remap_original_to_structured(
    ...
    Returns:
        ...
        active_conditional_slots: [B, 2], bool, active slot mask.
    """
